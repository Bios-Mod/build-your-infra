...
## Introduction
This document covers the OS hardening process applied before deploying any service.
...
The server is designed to run internet-facing and host real services (Nginx, BIND9,
Kea DHCP, Samba 4 AD) — base hardening is a non-negotiable prerequisite before
opening any port to the public network.

> **Note:** IP addresses shown throughout this repo (192.168.X.X) are placeholders.
> Replace with your actual values — LAN subnet for VM/bare metal, or the public IP
...
| Hypervisor       | VMware Fusion (macOS) · VMware / VirtualBox (Linux/Windows)           |
...
> - **Bare metal:** apply the Netplan config directly. Interface name will vary.
...
sudo cp no-dhcp.conf /etc/systemd/network/10-netplan-enp2s0.network.d/
...
```bash
sudo cp 99-wireguard.conf /etc/sysctl.d/99-wireguard.conf
...
| `[Peer] Endpoint` | LAN IP (VM/bare metal) or public IP (VPS) + port | `192.168.X.X:51820` |
...
sudo cp client-template.conf /etc/wireguard/wg0.conf
...
```bash
sudo cp 99-z-hardening.conf /etc/sysctl.d/99-z-hardening.conf
sudo sysctl --system
```
...
```bash
sudo cp disable-unused-protocols.conf /etc/modprobe.d/disable-unused-protocols.conf
...
sudo cp 99-hardening /etc/aide/aide.conf.d/99-hardening
...
```bash
sudo cp 20-ufw.conf         /etc/rsyslog.d/20-ufw.conf
sudo cp 99-hardening.conf   /etc/rsyslog.d/99-hardening.conf
sudo cp hardening-logs      /etc/logrotate.d/hardening-logs
...
> On bare metal or VPS, use your provider's snapshot/backup mechanism
> or a manual disk image before proceeding to service deployment.
